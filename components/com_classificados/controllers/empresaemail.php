 ...
 * @since    1.0.0
 */
class ClassificadosControllerEmpresaEmail extends BaseController
{

	const TB_EMAILEMPRESA = '`#__email_empresa`';
    ...
    public function apagar(){
        ...
        $query->update(ClassificadosControllerEmpresaEmail::TB_EMAILEMPRESA)->set($fields)->where($conditions);
        ...
        $app->redirect(JRoute::_( 'index.php?option=com_classificados&task=empresa.meusdados&t=email&Itemid='.$itemid , false ), "");
    }
    ...
    public function principal(){
		...
		$query->select('`a`.`email`')
			->from (ClassificadosControllerEmpresaEmail::TB_EMAILEMPRESA . ' AS `a`' )
			->where( '`a`.`status` = ' . $db->quote(ClassificadosControllerEmpresaEmail::STATUS_ATIVO), 'AND')
        ...
        if($email != null && $email != '' && $email->email != null && $email->email != ''){
            ...
            $query = $db->getQuery(true);
            $query->update(ClassificadosControllerEmpresaEmail::TB_USERS)->set($fields)->where($conditions);
    ...
    public function email(){
        ...
        $query = $db->getQuery ( true );
		$query->select(' `a`.`id`,`a`.`id_empresa`,`a`.`email`,`a`.`exibir`,`a`.`contato`,`a`.`validado`,`a`.`status`,`a`.`id_user_criador`,
			...
			->from (ClassificadosControllerEmpresaEmail::TB_EMAILEMPRESA . ' AS `a`' )
			->join ('LEFT', ClassificadosControllerEmpresaEmail::TB_USERS . ' AS `b` ON `a`.`id_user_alterador` = `b`.`id`')
			->where( '`a`.`status` = ' . $db->quote(ClassificadosControllerEmpresaEmail::STATUS_ATIVO), 'AND')
            ->where( '`a`.`id`  = ' . $db->quote($id))
            ->where( '`a`.`id_empresa`  = ' . $db->quote($user->id))
            ->setLimit(1);
    ...
    public function salvar(){
		...
		if($email==null || trim($email) == '' || strlen($email) < 3){
			JLog::add('Não enviou o e-mail do empresa', JLog::DEBUG, 'com-classificados-empresaemail');
			$app->enqueueMessage(JText::_('COM_CLASSIFICADOS_EMPRESA_EMAIL_OBRIGATORIO'), 'error');
			$isErro = true;
		}


        $exp = preg_match(ClassificadosControllerEmpresaEmail::REGEXP_EMAIL, $email);

        if($exp===false || $exp<=0) {
			JLog::add('Não enviou o e-mail invalido', JLog::DEBUG, 'com-classificados-empresaemail');
			$app->enqueueMessage(JText::_('COM_CLASSIFICADOS_EMPRESA_EMAIL_INVALIDO'), 'error');
			$isErro = true;
            ...
            if(!checkdnsrr ($host , "MX" )){
                JLog::add('Dominio não encontrado de e-mail', JLog::DEBUG, 'com-classificados-empresaemail');
                $app->enqueueMessage(JText::_('COM_CLASSIFICADOS_EMPRESA_EMAIL_DNS_INVALIDO'), 'error');
                $isErro = true;
            }
        ...
        $query->where( '`a`.`status` = ' . $db->quote(ClassificadosControllerEmpresaEmail::STATUS_ATIVO), 'AND')
        ...
        if($existe!= null && $existe->id != null  && $existe->id != ''  && $existe->id != 0 ){
			JLog::add('E-mail já cadastrado para essa empresa', JLog::DEBUG, 'com-classificados-empresaemail');
			$app->enqueueMessage(JText::_('COM_CLASSIFICADOS_EMPRESA_EMAIL_JA_EXISTE'), 'error');
			$isErro = true;
        }
        ...
        $query = $db->getQuery ( true );
		$query->select(' `a`.`id`')
			->from (ClassificadosControllerEmpresaEmail::TB_EMAILBLACKLIST . ' AS `a`' )
			->where( '`a`.`status` = ' . $db->quote(ClassificadosControllerEmpresaEmail::STATUS_ATIVO), 'AND')
        ...
        if($existe!= null && $existe->id != null  && $existe->id != ''  && $existe->id != 0 ){
			JLog::add('E-mail bloqueado', JLog::DEBUG, 'com-classificados-empresaemail');
			$app->enqueueMessage(JText::_('COM_CLASSIFICADOS_EMPRESA_EMAIL_BLOQUEADO'), 'error');
			$isErro = true;
        }
		...
		if($id != '' && $id != null && $id != 0 ){
            ...
            if($existe->email != $email ){
                ...
                if(!$this->_sendValidator($email, $uuid)){
                    JLog::add('Problema ao enviar o e-mail de validação', JLog::DEBUG, 'com-classificados-empresaemail');
                    $app->enqueueMessage(JText::_('COM_CLASSIFICADOS_EMPRESA_EMAIL_FALHA_ENVIO_VALIDACAO'), 'error');
                    $this->email();
                    return;
                }
            }
            ...
            $columns = array('email', 'exibir', 'contato', 'id_empresa','hash_validar',
            ...
            $values = array(
                ...
                $db->quote($user->id), 
                $db->quote($uuid),
                $db->quote(ClassificadosControllerEmpresaEmail::STATUS_ATIVO),
                $db->quote($user->id), 
            ...
            $query
                ->insert(ClassificadosControllerEmpresaEmail::TB_EMAILEMPRESA)
    ...
    private function _sendValidator($email, $uuid){
        ...
        $mailer->setSubject($app->getName() . ' : ' . JText::_('COM_CLASSIFICADOS_EMPRESA_EMAIL_VALIDACAO') );
        $mailer->isHtml(true);
        $mailer->Encoding = 'base64';



        $urlValidacao=Uri::root() .  JRoute::_( 'index.php?option=com_classificados&task=empresaemail.validar&email='.$email.'&h='. $uuid, false );

        $mailer->setBody(JText::sprintf('COM_CLASSIFICADOS_EMPRESA_EMAIL_TEXTO_EMAIL', $urlValidacao, $urlValidacao ) );
