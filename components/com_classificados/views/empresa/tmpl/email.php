...
$url =  JRoute::_( 'index.php?option=com_classificados&task=empresaemail.email&Itemid='.$itemid , false );
$urlGravar =  JRoute::_( 'index.php?option=com_classificados&task=empresaemail.salvar&Itemid='.$itemid , false );
$voltar = JRoute::_( 'index.php?option=com_classificados&task=empresa.meusdados&t=email&Itemid='.$itemid , false );
if ($task == null || $task == '') {
	$app->redirect ($url, "" );
	exit();
}
...
<form action="<?= $urlGravar ?>" method="post" enctype="application/x-www-form-urlencoded" class="form-validate form-horizontal">
	...
	<input type="hidden" name="task" value="empresaemail.salvar"/>
    ...
    <fieldset>
        ...
        <div class="control-group">
            <label class="control-label" for="email"><?= JText::_('COM_CLASSIFICADOS_EMPRESA_EMAIL') ?></label>
            <div class="controls">
                <input type="email" id="email" name="email" value="<?= $email ?>"
                    required="required" maxlength="200" minlength="3"
                    placeholder="<?= JText::_('COM_CLASSIFICADOS_EMPRESA_EMAIL_PLACEHOLDER') ?>"/>
            </div>
        </div>
        ...
        <div class="control-group">
            <label class="control-label" for="principal"><?= JText::_('COM_CLASSIFICADOS_EMPRESA_EMAIL_TORNAR_PRINCIPAL') ?></label>
            <div class="controls">
        ...
        <div class="control-group">
            <label class="control-label" for="exibir"><?= JText::_('COM_CLASSIFICADOS_EMPRESA_EXIBIR_EMAIL') ?></label>
            <div class="controls">
                <input type="checkbox" name="exibir" id="exibir" value="1" <?= $this->checked($input->get('exibir'), 'S' )?>/>
            </div>
        </div>
        <div class="control-group">
            <label class="control-label" for="controle"><?= JText::_('COM_CLASSIFICADOS_EMPRESA_CONTROLE_EMAIL') ?></label>
            <div class="controls">
