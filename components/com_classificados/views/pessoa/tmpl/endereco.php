...
$endereco = $input->get('endereco',null,'string');
...
jQuery(document).ready(function(){
    jQuery("#endereco").mask("999999999");
    jQuery("#ddd").mask("999")
});');
?>
<form action="<?= $urlGravar ?>" method="post" enctype="application/x-www-form-urlencoded" class="form-validate form-horizontal">
    ...
    <fieldset>
        ...
        <div class="control-group">
            <label class="control-label" for="exibir"><?= JText::_('COM_CLASSIFICADOS_PESSOA_EXIBIR_endereco') ?></label>
            <div class="controls">
                <input type="checkbox" name="exibir" id="exibir" value="1" <?= $this->checked($input->get('exibir'), 'S' )?>/>
            </div>
        </div>
        <div class="control-group">
            <label class="control-label" for="controle"><?= JText::_('COM_CLASSIFICADOS_PESSOA_TIPO_endereco') ?></label>
            <div class="controls">
