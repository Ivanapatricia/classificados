...
if($itens!=null) : ?>
            <table class="table table-striped table-hover" title="<?=  JText::_('COM_CLASSIFICADOS_PESSOA_LISTA_ENDERECOS');?>">
                ...
                <tr>
                    ...
                    <th id="editar_col"><?=  JText::_('COM_CLASSIFICADOS_PESSOA_EXIBIR_ENDERECO');?></th>
                </tr>
                </thead>
                <tbody>
<?php
        ...
        ?>	<tr>
				...
				<td><a href="javascript: if(confirm('<?= 
					JText::sprintf('COM_CLASSIFICADOS_INFORMACOES_CONFIRMA_REMOCAO', $item->ddd . ' ' .  $item->endereco)  ;
				?>')){ window.location='<?=  $urlRemover;?>'}"
				       class="text-error"  title="<?= JText::_('COM_CLASSIFICADOS_BTN_REMOVER_TEXT') ?>"><em
								class="icon-remove"></em></a></td>
                <td><?= JHTML::tooltip($tooltipo, JText::_('COM_CLASSIFICADOS_INFORMACOES'), '', '('. $item->ddd . ') ' . $item->endereco ) ?></td>
