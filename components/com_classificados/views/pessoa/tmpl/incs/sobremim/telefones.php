...
jQuery(document).ready(function(){
    jQuery("#btnBuscar").click(function(){
        window.location=("'   . JRoute::_( 'index.php?option=com_classificados&task=pessoatelefone.telefone&Itemid='.$itemid , false ) . '");
...
if($itens!=null) : ?>
            <table class="table table-striped table-hover" title="<?=  JText::_('COM_CLASSIFICADOS_PESSOA_LISTA_TELEFONES');?>">
                <caption><?=  JText::_('COM_CLASSIFICADOS_PESSOA_LISTA_TELEFONES');?></caption>
                <thead>
                <tr>
                    <th id="excluir_col" class="span1">&nbsp;</th>
                    <th id="editar_col" class="span1">&nbsp;</th>
                    <th id="editar_col"><?=  JText::_('COM_CLASSIFICADOS_PESSOA_TELEFONE');?></th>
                    <th id="editar_col"><?=  JText::_('COM_CLASSIFICADOS_PESSOA_EXIBIR_TELEFONE');?></th>
                </tr>
                </thead>
                <tbody>
<?php
		...
		foreach ($itens as $item) :
			$urlEditar = JRoute::_ ( 'index.php?option=com_classificados&task=pessoatelefone.telefone&id='.$item->id.'&Itemid=' . $itemid, false );
			$urlRemover = JRoute::_ ( 'index.php?option=com_classificados&task=pessoatelefone.apagar&id='.$item->id.'&Itemid='. $itemid, false );
...
<div class="btn-toolbar text-right">
    <div class="btn-group">
        <a type="button" id="btnBuscar" class="btn btn-success" href="<?= JRoute::_( 'index.php?option=com_classificados&task=pessoatelefone.telefone&Itemid='.$itemid , false )  ?>"><?=
